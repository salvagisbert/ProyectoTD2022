...
El DataFrame "datos" tiene 8 variables que son id, tiempo_grabacion, r_continuo, r_7a19, r_tardenoche, r_19a23, r_23a7 y fecha_observacion.
...
En este Data Frame observamos los datos recogidos por sensores que monitorizan el nivel de ruido en diferentes localizaciones del barrio de Ruzafa.

4) ¿Hay datos perdidos?¿Es un analisis univariante o bivariante?

Si que hay datos perdidos y, es un análisis bivariante porque hay más de una variable.
...
#Fase 3. Acondicionamiento en tidy dataset
...
```{r}
colnames(datos)<- c("id", "tiempo_grabacion", "r_continuo","r_7a19", "r_tardenoche", "r_19a23", "r_23a7","fecha_observacion")
```
...
```{r}
gather(datos, key= Horario, value= Nivel_Sonoro ,4:8)
```